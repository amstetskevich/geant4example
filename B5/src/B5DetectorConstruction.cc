...
G4ThreadLocal PurgMagTabulatedField3D* B5DetectorConstruction::fMagneticField = 0;
...
: G4VUserDetectorConstruction(), 
  fMessenger(nullptr),
  //fHodoscope1Logical(nullptr),
  fHodoscope2Logical(nullptr),
...
G4VPhysicalVolume* B5DetectorConstruction::Construct()
{
  ...
  auto argonGas = G4Material::GetMaterial("G4_Galactic");
  ...
  auto magneticSolid 
    = new G4Orb("magneticTubs", 0.5*m);
                //G4Tubs("magneticTubs",0.,1.*m,1.*m,0.,360.*deg);
  ...
  G4UserLimits* userLimits = new G4UserLimits(0.04*m);
...
//  fHadCalScintiLogical->SetSensitiveDetector(hadCalorimeter);
  ...
  // magnetic field ----------------------------------------------------------
  const char* filename ="PurgMag3D.TABLE";
  fMagneticField = new PurgMagTabulatedField3D(filename, 0);
